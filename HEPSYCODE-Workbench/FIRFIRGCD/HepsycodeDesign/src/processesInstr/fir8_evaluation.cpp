...
/********************************************************************************
 * fir8_evaluatin : FirFirGCD FIR8 Evaluation process.
 *
 * FIR_8 Evaluation process: CSP Process specifying the functionality of
 * the FIR16 Evaluation component
 *
********************************************************************************/

/*! \file fir8_evaluation.cpp
    \brief FIR8 Evaluation Documented file.

    FIR_8 Evaluation process: CSP Process specifying the functionality of
    the FIR8 Evaluation component
*/
...
void mainsystem::fir8_evaluation()
{
	...
	HEPSY_S(fir8e_id) while(1) //HEPSY_S rules for WHILE: before while, and after {
	{HEPSY_S(fir8e_id)
		...
		HEPSY_S(fir8e_id) for (unsigned j = 0; j < TAP8; j++)
		{HEPSY_S(fir8e_id)
			HEPSY_S(fir8e_id)coef[j] = fir8e_p.coef[j];
		HEPSY_S(fir8e_id)}

		HEPSY_S(fir8e_id) for (unsigned j = 0; j < TAP8; j++)
		{HEPSY_S(fir8e_id)
				HEPSY_S(fir8e_id) shift[j] = fir8e_p.shift[j];
		HEPSY_S(fir8e_id)}

		// process
		HEPSY_S(fir8e_id) acc=sample_tmp*coef[0];

		HEPSY_S(fir8e_id) for(int i=TAP8-2; i>=0; i--) //HEPSY_S rules for FOR: before for, after {, and first of }
		{HEPSY_S(fir8e_id)
