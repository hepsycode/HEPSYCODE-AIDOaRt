...
void ann_04_genann_init_sigmoid_lookup(const ann_04_genann *ann) {
        const double f = (ann_04_sigmoid_dom_max - ann_04_sigmoid_dom_min) / ann_04_LOOKUP_SIZE;
        int i;

        ann_04_interval = ann_04_LOOKUP_SIZE / (ann_04_sigmoid_dom_max - ann_04_sigmoid_dom_min);
        for (i = 0; i < ann_04_LOOKUP_SIZE; ++i) {
        	ann_04_lookup[i] = ann_04_genann_act_sigmoid(ann, ann_04_sigmoid_dom_min + f * i);
        }
}
...
double ann_04_genann_act_sigmoid_cached(const ann_04_genann *ann unused, double a) {
    ...
    if (a < ann_04_sigmoid_dom_min) return ann_04_lookup[0];
    if (a >= ann_04_sigmoid_dom_max) return ann_04_lookup[ann_04_LOOKUP_SIZE - 1];

    size_t j = (size_t)((a-ann_04_sigmoid_dom_min)*ann_04_interval+0.5);

    /* Because floating point... */
    if (unlikely(j >= ann_04_LOOKUP_SIZE)) return ann_04_lookup[ann_04_LOOKUP_SIZE - 1];

    return ann_04_lookup[j];
}
...
ann_04_genann *ann_04_genann_init(int inputs, int hidden_layers, int hidden, int outputs) {
    ...
    const int size = sizeof(ann_04_genann) + sizeof(double) * (total_weights + total_neurons + (total_neurons - inputs));
    ann_04_genann *ret = (ann_04_genann *)malloc(size);
    if (!ret) return 0;
    ...
    /* Set pointers. */
    ret->weight = (double*)((char*)ret + sizeof(ann_04_genann));
    ret->output = ret->weight + ret->total_weights;
    ret->delta = ret->output + ret->total_neurons;

    ann_04_genann_randomize(ret);

    ret->activation_hidden = ann_04_genann_act_sigmoid_cached;
    ret->activation_output = ann_04_genann_act_sigmoid_cached;

    ann_04_genann_init_sigmoid_lookup(ret);
...
double const *ann_04_genann_run(ann_04_genann const *ann, double const *inputs) {
    ...
    for (j = 0; j < ann->hidden; ++j) {
        ...
        *o++ = ann_04_genann_act_hidden(ann, sum);
    }
...
int ann_04_annCreate(int n)
{
	...
	if	( (ann_04_nANN != -1)  || (n <= 0) || (n > ann_04_MAX_ANN) )
		return( EXIT_FAILURE );
	...
	for	( int i = 0; i < n; i++ )
	{
		...
		ann_04_ann[i] = ann_04_genann_init(ann_04_ANN_INPUTS, ann_04_ANN_HIDDEN_LAYERS, ann_04_ANN_HIDDEN_NEURONS, ann_04_ANN_OUTPUTS);
		if	( ann_04_ann[i] == 0 )
		{
			for	(int j = 0; j < i; j++)
				ann_04_genann_free(ann_04_ann[j]) ;
			return( EXIT_FAILURE );
		}
	}
...
{
	if	( ann_04_annCreate(n) != EXIT_SUCCESS )
		return( EXIT_FAILURE );

    // Set weights
	for	( int index = 0; index < ann_04_nANN; index++ )
		for	( int i = 0; i < ann_04_ann[index]->total_weights; ++i )
			ann_04_ann[index]->weight[i] = ann_04_weights[i] ;
...
{
	if	( ann_04_annCheck(index) != EXIT_SUCCESS )
		return( EXIT_FAILURE ) ;

	x[2] = * ann_04_genann_run(ann_04_ann[index], x) ;
...
void ann_04_annDestroy(void)
{
	...
	for	( int index = 0; index < ann_04_nANN; index++ )
		ann_04_genann_free(ann_04_ann[index]) ;
...
void mainsystem::ann_04_main()
{
	...
	if	( ann_04_annCreateAndSetWeights(ann_04_NUM_DEV) != EXIT_SUCCESS ){		// Create and init ANN
	...
	{HEPSY_S(ann_04_id)
		...
		cleanData_xx_ann_xx_payload_var = cleanData_04_ann_04_channel->read();
...
//		}
		//u: cycle num
		if	( ann_04_annRun(0, x) != EXIT_SUCCESS ){
		...
		ann_04_dataCollector_channel->write(ann_xx_dataCollector_payload_var);
