...
void ann_02_genann_init_sigmoid_lookup(const ann_02_genann *ann) {
        const double f = (ann_02_sigmoid_dom_max - ann_02_sigmoid_dom_min) / ann_02_LOOKUP_SIZE;
        int i;

        ann_02_interval = ann_02_LOOKUP_SIZE / (ann_02_sigmoid_dom_max - ann_02_sigmoid_dom_min);
        for (i = 0; i < ann_02_LOOKUP_SIZE; ++i) {
        	ann_02_lookup[i] = ann_02_genann_act_sigmoid(ann, ann_02_sigmoid_dom_min + f * i);
        }
}
...
double ann_02_genann_act_sigmoid_cached(const ann_02_genann *ann unused, double a) {
    ...
    if (a < ann_02_sigmoid_dom_min) return ann_02_lookup[0];
    if (a >= ann_02_sigmoid_dom_max) return ann_02_lookup[ann_02_LOOKUP_SIZE - 1];

    size_t j = (size_t)((a-ann_02_sigmoid_dom_min)*ann_02_interval+0.5);

    /* Because floating point... */
    if (unlikely(j >= ann_02_LOOKUP_SIZE)) return ann_02_lookup[ann_02_LOOKUP_SIZE - 1];

    return ann_02_lookup[j];
}
...
ann_02_genann *ann_02_genann_init(int inputs, int hidden_layers, int hidden, int outputs) {
    ...
    const int size = sizeof(ann_02_genann) + sizeof(double) * (total_weights + total_neurons + (total_neurons - inputs));
    ann_02_genann *ret = (ann_02_genann *)malloc(size);
    if (!ret) return 0;
    ...
    /* Set pointers. */
    ret->weight = (double*)((char*)ret + sizeof(ann_02_genann));
    ret->output = ret->weight + ret->total_weights;
    ret->delta = ret->output + ret->total_neurons;

    ann_02_genann_randomize(ret);

    ret->activation_hidden = ann_02_genann_act_sigmoid_cached;
    ret->activation_output = ann_02_genann_act_sigmoid_cached;

    ann_02_genann_init_sigmoid_lookup(ret);
...
double const *ann_02_genann_run(ann_02_genann const *ann, double const *inputs) {
    ...
    if (!ann->hidden_layers) {
        ...
        for (j = 0; j < ann->outputs; ++j) {
            ...
            *o++ = ann_02_genann_act_output(ann, sum);
        }
    ...
    for (j = 0; j < ann->hidden; ++j) {
        ...
        *o++ = ann_02_genann_act_hidden(ann, sum);
    }
...
int ann_02_annCreate(int n)
{
	...
	if	( (ann_02_nANN != -1)  || (n <= 0) || (n > ann_02_MAX_ANN) )
		return( EXIT_FAILURE );
	...
	for	( int i = 0; i < n; i++ )
	{
		...
		ann_02_ann[i] = ann_02_genann_init(ann_02_ANN_INPUTS, ann_02_ANN_HIDDEN_LAYERS, ann_02_ANN_HIDDEN_NEURONS, ann_02_ANN_OUTPUTS);
		if	( ann_02_ann[i] == 0 )
		{
			for	(int j = 0; j < i; j++)
				ann_02_genann_free(ann_02_ann[j]) ;
			return( EXIT_FAILURE );
		}
	}
...
{
	if	( ann_02_annCreate(n) != EXIT_SUCCESS )
		return( EXIT_FAILURE );

    // Set weights
	for	( int index = 0; index < ann_02_nANN; index++ )
		for	( int i = 0; i < ann_02_ann[index]->total_weights; ++i )
			ann_02_ann[index]->weight[i] = ann_02_weights[i] ;
...
{
	if	( ann_02_annCheck(index) != EXIT_SUCCESS )
		return( EXIT_FAILURE ) ;

	x[2] = * ann_02_genann_run(ann_02_ann[index], x) ;
...
void ann_02_annDestroy(void)
{
	...
	for	( int index = 0; index < ann_02_nANN; index++ )
		ann_02_genann_free(ann_02_ann[index]) ;
...
void mainsystem::ann_02_main()
{
	...
	if	( ann_02_annCreateAndSetWeights(ann_02_NUM_DEV) != EXIT_SUCCESS ){		// Create and init ANN
	...
	{HEPSY_S(ann_02_id)
		...
		cleanData_xx_ann_xx_payload_var = cleanData_02_ann_02_channel->read();
...
//		}
		//u: cycle num
		if	( ann_02_annRun(0, x) != EXIT_SUCCESS ){
		...
		ann_02_dataCollector_channel->write(ann_xx_dataCollector_payload_var);
