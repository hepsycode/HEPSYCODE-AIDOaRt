...
//	{
		device.t = cD_04_T_0 ;		// Operating temperature = Ambient temperature
		device.r = cD_04_R_0 ;		// Operating R_DS_On = Data-sheet R_DS_On
		// printf("Init %d \n",i);
//	}
...
void cD_04_cleanData(int index, double sample[cD_04_SAMPLE_LEN])
{
	...
	if	( sample[cD_04_I_INDEX] < 0 )
		sample[cD_04_I_INDEX] = 0 ;
	else if ( sample[cD_04_I_INDEX] > (2.0 * cD_04_I_0) )
		sample[cD_04_I_INDEX] = (2.0 * cD_04_I_0) ;
	// Postcondition: (0 <= sample[I_INDEX] <= 2.0*I_0)

	if	( sample[cD_04_V_INDEX] < 0 )
		sample[cD_04_V_INDEX] = 0 ;
	else if ( sample[cD_04_V_INDEX] > (2.0 * cD_04_V_0 ) )
		sample[cD_04_V_INDEX] = (2.0 * cD_04_V_0) ;
...
double cD_04_degradationModel(double tNow)
{
	...
	//r = R_0 + Alpha * tNow ;
	r = cD_04_R_0 * ( 2 - exp(-cD_04_Alpha*tNow) );
...
void mainsystem::cleanData_04_main()
{
	...
	cD_04_device.t = cD_04_T_0 ;		// Operating temperature = Ambient temperature
	cD_04_device.r = cD_04_R_0 ;		// Operating R_DS_On = Data-sheet R_DS_On

	// ### added for time related dynamics
	//static double cD_04_Tau = 0.02 ;				// [s]		Sampling period
	double cD_04_last_sample_time = 0;
	...
	{HEPSY_S(cleanData_04_id)
		...
		sampleTimCord_cleanData_xx_payload_var = sampleTimCord_cleanData_04_channel->read();
...
//				<< endl;
		...
		// reconstruct sample
		cD_04_sample[cD_04_I_INDEX] = sample_i;
		cD_04_sample[cD_04_V_INDEX] = sample_v;
		cD_04_sample[cD_04_TIME_INDEX] = ex_time;



		// ###   C L E A N   D A T A   (0 <= I <= 2.0*I_0) and (0 <= V <= 2.0*V_0)
		cD_04_cleanData(dev, cD_04_sample) ;
		cD_04_device.time_Ex = cD_04_sample[cD_04_TIME_INDEX] ;
		cD_04_device.i = cD_04_sample[cD_04_I_INDEX] ;
		cD_04_device.v = cD_04_sample[cD_04_V_INDEX] ;

		// ### added for time related dynamics
		//static double cD_04_Tau = 0.02 ;				// [s]		Sampling period
		cD_04_Tau = ex_time - cD_04_last_sample_time;
		cD_04_last_sample_time = ex_time;

		// ###   F E A T U R E S   E X T R A C T I O N   (compute the temperature)
		cD_04_device.t = cD_04_extractFeatures(	cD_04_device.t,	// Previous temperature
		...
		cD_04_device.r = cD_04_degradationModel(cD_04_device.t) ;

		// ###   N O R M A L I Z E:  (x[0] in {0.0, 1.0}) and (x[1] in {0.0, 1.0})
		x[0] = cD_04_device.i ;
		x[1] = cD_04_device.v ;
		cD_04_normalize(dev, x) ;
...
//		if	( annRun(index, x) != EXIT_SUCCESS ){
		...
		cleanData_xx_ann_xx_payload_var.device_i = cD_04_device.i;
		cleanData_xx_ann_xx_payload_var.device_v = cD_04_device.v;
		cleanData_xx_ann_xx_payload_var.device_t = cD_04_device.t;
		cleanData_xx_ann_xx_payload_var.device_r = cD_04_device.r;
		cleanData_xx_ann_xx_payload_var.x_0 = x[0];
		cleanData_xx_ann_xx_payload_var.x_1 = x[1];
		cleanData_xx_ann_xx_payload_var.x_2 = x[2];


		cleanData_04_ann_04_channel->write(cleanData_xx_ann_xx_payload_var);
